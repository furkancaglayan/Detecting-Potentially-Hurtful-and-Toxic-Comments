...
class AdaBoostDecisionTree(_AdaBoostClassifier):
    def __init__(self, max_tree_depth=8, n_estimators=100, lr=1):
        clf_base = AdaBoostClassifier(
            DecisionTreeClassifier(max_depth=max_tree_depth),
            n_estimators=n_estimators, learning_rate=lr, algorithm="SAMME"
        )
        clf = Pipeline([
            ('vectorizer', TfidfVectorizer(stop_words=ENGLISH_STOP_WORDS)),
            ('clf', clf_base),
...
class AdaBoostNaiveBayes(_AdaBoostClassifier):
    def __init__(self, n_estimators=100, lr=1):
        clf_base = AdaBoostClassifier(
...
class AdaBoostSVM(_AdaBoostClassifier):
    def __init__(self, n_estimators=100, lr=1):
        ...
        super().__init__("AdaBoost Classifier, SVM with {} estimators".format(n_estimators), clf)
